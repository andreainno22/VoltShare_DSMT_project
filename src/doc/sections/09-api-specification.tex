...
two gives the 90~s. An earlier version had both timers computing the whole budget, and a
mute charge point stayed reservable for three minutes.

\paragraph{Four ways the socket closes.} The permanent one is decided at the handshake, when
the query string is unreadable, or names another station, or names a connector this station
does not own. A second socket booting for the same connector evicts the first, and the
eviction waits for the \texttt{boot}, so that a half-open connection cannot throw out a
working charge point before saying a word. The station shutting down closes after its
\texttt{stop} frame. The fourth is the connector process dying under a socket that is
perfectly healthy: the socket waits a few seconds for the supervisor to put a replacement in
its place, hands it what it knows about the connector, and closes only if none arrives.
...
\begin{itemize}
  \item \textbf{A subset of OCPP 1.6-J, with the same direction of authority.} Full OCPP
        ...
        nothing to be an emulator of. Message by message: \texttt{boot} is
        \texttt{BootNotification} without the configuration keys, \texttt{status} is
        \texttt{StatusNotification} with nine states cut to four, \texttt{plugged} is
        \texttt{Authorize} plus \texttt{StartTransaction} with the vehicle identifying itself
        instead of an id tag, \texttt{meter} is \texttt{MeterValues} down to three
        measurands, \texttt{unplugged} is \texttt{StopTransaction}, \texttt{set\_limit} is
        \texttt{SetChargingProfile} stripped of profiles and stacking levels, and
        \texttt{stop} is \texttt{RemoteStopTransaction}. The four operations OCPP adds for
        firmware, resets and cable release have no counterpart here.
