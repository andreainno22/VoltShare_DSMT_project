...
| **P4** | A station node crashes, or is partitioned from the rest of the network | Across nodes | Node monitoring detects the failure; pending reservations on that station decay by lease; sessions in progress are reconciled for billing on restart; drivers are freed to reserve elsewhere |
