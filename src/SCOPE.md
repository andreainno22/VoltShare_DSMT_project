...
**A station has two distinct failures, not one**, and P4 covers both. The node can die — sessions lost, nothing written — or its uplink can be cut, in which case the site goes on delivering power while the operator's view of it disappears. The second is the one the lease and the node monitor exist for, and it is the one with a plain reading outside this room. They are demonstrated separately, with `docker kill` and `docker network disconnect`.
...
That requirement is tested rather than merely asserted. Cutting a station off the cluster network with `docker network disconnect` leaves the site delivering power while the operator loses sight of it: measured on 3 September, energy climbing inside the isolated station (0.248 → 0.403 → 0.558 kWh) while the coordinator logged `Node vs@station2 not responding`, dropped its claims, and the lobby stopped listing it. Reconnecting restored the operator's view without interrupting either charge. One limit of the same experiment, worth stating: the isolated station finishes what it started but can start nothing new, because authorising a car means resolving vehicle → owner in MySQL, which is on the other side of the cut.
